...
export function initPricingPage() {
    ...
    if (getUser()?.role !== 'vendor') {
        root.innerHTML = `
            <div class="border border-amber-200 bg-amber-50 rounded-lg p-6 text-center">
                ...
                <a href="/browse" class="inline-block mt-4 text-sm font-medium text-emerald-700 hover:underline">Back to browsing</a>
            </div>`;
        return;
    }
    ...
    // ── Espees ───────────────────────────────────────────────────────
    document.getElementById('espees-form')?.addEventListener('submit', async (e) => {
        e.preventDefault();
        const errorEl = document.getElementById('espees-error');
        const successEl = document.getElementById('espees-success');
        errorEl.classList.add('hidden');
        successEl.classList.add('hidden');

        const fd = new FormData(e.target);
        try {
            const data = await apiFetch('/subscriptions/espees/pay', {
                method: 'POST',
                json: {
                    wallet_id: fd.get('wallet_id'),
                    pin: fd.get('pin'),
