        ...
        try {
            ...
            listEl.innerHTML = chats
                ...
                >
                    ...
                    ${c.unread_count > 0 ? `<span class="w-5 h-5 rounded-full bg-orange-600 text-white text-xs flex items-center justify-center shrink-0">${c.unread_count}</span>` : ''}
                </button>`
                )
                .join('');
        ...
        messagesEl.innerHTML = messages
            .map((m) => {
                ...
                <div class="flex ${mine ? 'justify-end' : 'justify-start'} mb-2">
                    <div class="max-w-[75%] rounded-lg px-3 py-2 text-sm ${mine ? 'bg-orange-600 text-white' : 'bg-gray-100 text-gray-900'}">
