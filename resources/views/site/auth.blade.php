<x-site-layout title="Sign in — Sbrai Solutions" page="auth">
    ...
    <div class="relative min-h-screen flex items-center justify-center overflow-hidden bg-gray-50 px-4 sm:px-6 py-12">
        ...
        <div class="relative z-10 w-full max-w-md">
            ...
            <div class="auth-card bg-white rounded-3xl shadow-2xl shadow-gray-900/10 border border-gray-200 p-8 sm:p-10">
                ...
                {{-- ── Icon + heading (dynamic per panel/role) ──────────── --}}
                ...
                <div data-auth-panel="login">
                    <form id="login-form" class="space-y-4">
                        ...
                        {{-- Buyer / Vendor toggle — UI-only for now (see note below the
                             form): Api\AuthController::login() doesn't take a role param,
                             it just authenticates and returns whatever role the account
                             already has. This drives the heading copy today; wire it into
                             the submit payload later if you want the backend to care. --}}
                        ...
                        {{-- Social sign-in (Google) — Google renders its own button into this
                             container; it can't render inside a custom <button> with our own
                             markup, so this must be a plain div with the client id attached. --}}
                        <div class="space-y-3">
                            <div
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <a href="{{ route('password.request') }}" class="auth-link text-xs font-semibold text-orange-600 normal-case tracking-normal">Forgot password?</a>
                            </div>
                ...
                <div data-auth-panel="register" class="hidden">
                    <form id="register-form" class="space-y-4">
                        ...
                        {{-- Vendor-only fields — your backend requires business_name when
                             role=vendor (AuthController@register: required_if:role,vendor).
                             Hidden for buyers, shown once "Vendor" is selected above. --}}
            ...
            {{-- ── Verification disclaimer (register only) ──────────────── --}}
