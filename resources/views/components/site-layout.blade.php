...
<body data-page="{{ $page ?? '' }}" class="font-sans antialiased bg-gray-50 text-gray-900 min-h-screen flex flex-col">
    ...
    <header class="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 h-16 flex items-center gap-6">
            ...
            <div class="flex items-center gap-2 ml-auto">
                ...
                {{-- Desktop auth cluster: breakpoint handled by the WRAPPER
                     (.desktop-auth-cluster), not by the elements below —
                     data-nav="guest"/"user" keep their original, untouched
                     classes so site.js's hidden/flex toggle works exactly
                     as it always did. --}}
                <div class="desktop-auth-cluster">
                    {{-- Static "flex" removed — nav.js now toggles "flex"
                         in lockstep with "hidden" for this element too
                         (see nav.js render()), so it doesn't fight the
                         JS-added "hidden" class the same way "user" did. --}}
                    ...
                    {{-- ROOT CAUSE: this div never had "flex" in its class
                         list — only "hidden" plus flex-child utilities
                         (items-center, gap-*) that do nothing without a
                         flex container. nav.js only toggled "hidden" off,
                         so the div fell back to display:block and its two
                         <details> children (bell, account) stacked
                         vertically instead of sitting in a row.
                         FIX: nav.js now toggles "flex" on/off in lockstep
                         with "hidden" (see site.js nav.js render()) rather
                         than relying on a static "flex" class — this repo's
                         Tailwind build order makes ".flex" beat ".hidden"
                         when both are present statically (see the
                         .desktop-auth-cluster comment above for the same
                         issue), so they must never both sit on the class
                         list at once. Do not add a static "flex" class
                         here. --}}
    ...
    <footer class="bg-white border-t border-gray-200 mt-16">
        ...
        {{-- ── Newsletter strip — py-8 → py-5 to shrink footer height ── --}}
    ...
    <button type="submit" class="footer-newsletter-btn shrink-0 bg-orange-600 hover:bg-orange-700 text-white text-sm font-semibold px-5 py-2.5 rounded-xl shadow-sm">
        ...
        {{-- ── Main footer columns — now uses class="footer-flex" (flex
             layout) instead of the old grid. py-14 → py-8 to reduce
             footer height. --}}
        ...
        <div class="border-t border-gray-100">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex flex-col-reverse md:flex-row items-center justify-between gap-4">
                <p class="text-xs text-gray-400 text-center md:text-left">
                    &copy; {{ date('Y') }} Sbrai Solutions. All rights reserved.
                </p>
