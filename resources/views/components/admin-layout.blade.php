    ...
    <div class="flex min-h-screen">
        ...
        <aside id="sidebar"
            ...
            <a href="{{ route('admin.support.index') }}"
                class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-100 transition {{ request()->routeIs('admin.support*') ? 'active' : '' }}">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex-1 w-full lg:ml-64 min-w-0">
            <header
                ...
                <div class="flex items-center gap-3 min-w-0">
                    ...
                    <div class="min-w-0">
                        <h1 class="text-base sm:text-lg font-bold text-gray-900 truncate">{{ $title ?? 'Dashboard' }}
                        </h1>
                        <p class="text-xs text-gray-400 truncate hidden sm:block">
                            {{ $subtitle ?? 'Manage Sbrai Hub' }}</p>
                    </div>
                </div>
